...
STEP_BEGIN
	...
	if (STEP_NUMBER == 1)
	{
		...
		// Get current limit of axis 1.
		EXTRA(1,0) = EXTRA(0,0).getCurrentLimit(1,1);
	...
	// Signal on output #1 whether ARM 1 is driven on and in mode 9.
	if (EXTRA(0,0).isInDriveOn(1) && EXTRA(0,0).getMode(1) == C4G_OPEN_MODE_9) OUTPUT(1,0) = 1;
	else OUTPUT(1,0) = -1;
	
	// Write axis 1 current limit (in ampere) to output #2.
	OUTPUT(2,0) = EXTRA(1,0);
STEP_END
