...
INITIALIZE_BEGIN
	...
	ALLOCATE_EXTRA(1, 6, double);    // Gear ratios 
INITIALIZE_END
...
STEP_BEGIN
	...
	if (STEP_NUMBER == 1)
	{
		...
		// Compute gear ratios.
		for (unsigned int i = 0; i < 6; i++) EXTRA(1,i) = EXTRA(0,0).getTxRate(1,i+1) / 360.0;
	...
	// Signal on output #1 whether ARM 1 is driven on and in mode 5.
	if (EXTRA(0,0).isInDriveOn(1) && EXTRA(0,0).getMode(1) == C4G_OPEN_MODE_5) OUTPUT(1,0) = 1;
	else OUTPUT(1,0) = -1;

	// Write actual positions (in deg) to output #2.
	for (unsigned int i = 0; i < 6; i++) OUTPUT(2,i) = EXTRA(0,0).getActualPosition(1, i+1) / EXTRA(1,i);
STEP_END
