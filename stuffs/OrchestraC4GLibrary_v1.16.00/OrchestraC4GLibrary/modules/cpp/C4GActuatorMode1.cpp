...
STEP_BEGIN
	...
	if (c4gOpen->getMode(ARM) == C4G_OPEN_MODE_1)
	{
		for (unsigned int i = 0; i < NUM_OF_AXES; i++)
		{
			c4gOpen->setTargetPosition(ARM, i+1, INPUT(1,i));
			c4gOpen->setTargetVelocity(ARM, i+1, INPUT(2,i));
			c4gOpen->setFeedForwardVelocity(ARM, i+1, INPUT(3,i));
			c4gOpen->setFeedForwardCurrent(ARM, i+1, INPUT(4,i));
		}
	}
