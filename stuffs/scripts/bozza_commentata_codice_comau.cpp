...
class Server {
	...
	while (serverActive) {
		...
		//size = recvfrom(udpSocket, mesg, 56, 0,(struct sockaddr *)&clientName, &clientLength);
		/*if (size == -1) {
			perror("recvfrom()");
			exit(1);
		}*/  //END if (size == -1)
		...
		command=(int)mesg[0];
 		...
 		for (int i = 0; i < 6; i++) arg[i] = mesg[i+1];
	...
	}//END while (serverActive)
	}//END void StartRECEIVE()
};//END class Server
...
int main(int argc, char *argv[])
{
	...
	//statusSend = bind(udpSocketSend, (struct sockaddr *)&to_simulink,sizeof(to_simulink));
