...
bool AXIS_ENABLED[6];
double AXIS_SPEED[6] = {0,0,0,0,0,0}; //speed in delta deg/cycle
float setPoint[6] = {0,0,0,0,0,0};
double getActualGears[6] = {1,1,0,0,0,0};
//FILTER VARS
...
float FcMax = 3;
float FcMin= 0.25;
float threshold_base[6] = {5,5,5,10,5,10};
		...
		if (command==3) 	{if ((float)arg[0]<=10.0 && (float)arg[0]>0.1 && (float)arg[0]>(float)arg[1]) {
							...
							for (int i = 0; i < 6; i++) {
								...
								setPoint[i] = (float)(arg[i]);
                                			...
                                			//Check if point is relatively far from current position
								float err = abs(getActualGears[i]-setPoint[i]);
                                
                               				 //update threshold for this axis relatively to the current axis speed
								double speed = abs(float(AXIS_SPEED[i]));
				
                                				float threshold;
								if (speed <= 0.000001) threshold = 0.0;
									else threshold = threshold_base[i];

							    	if (err >= threshold) {
                                        				farPoint=true;
                                        				float relative = (threshold/err);
				     ...
				     //cout << " " << (speed);	
							...
							//cut-off frequency saturations and adjustements
...
int main(int argc, char *argv[])
{
	...
	if (c4gOpen.start())
	{
		...
		float VT_LIM[6][2] = {{-170,170},{-85,175},{-170,0},{-210,210},{-100,100},{-360,360}};
		...
		while (keepGoingOn)
		{
			...
			getActualGears[i] = -calibCONSTANTS[i]*360.0/txRate[i] + c4gOpen.getActualPosition(ARM, i+1)*360.0/txRate[i];
			AXIS_SPEED[i] = c4gOpen.getActualVelocity(ARM,i+1)*360.0/txRate[i];
				...
				if (setPoint[i]>=(VT_LIM[i][1]-1) && lastDeg>=(VT_LIM[i][1]-1) && lastDeg<setPoint[i]) {
					...
					setPoint[i] = lastDeg + corrective*abs(setPoint[i]-lastDeg);
						...
						size=sendto(udpSocketSend, getActualGears, 48, 0,(struct sockaddr *)&to_simulink, to_simulinkLength);
						if (size == -1) {
							perror("sendto()");
							exit(1);
						}
