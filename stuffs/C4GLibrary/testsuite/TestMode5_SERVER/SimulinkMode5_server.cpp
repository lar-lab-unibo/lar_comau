...
bool AXIS_ENABLED[6];
float AXIS_SPEED[6] = {0,0,0,0,0,0}; //speed in delta deg/cycle
...
float FcMax = 5;
float FcMin= 0.25;
float threshold_base[6] = {5,5,5,20,5,20};
	...
	while (serverActive) {
		...
		if (command==3) 	{if ((float)arg[0]<=10.0 && (float)arg[0]>0.1 && (float)arg[0]>(float)arg[1]) {
							...
							for (int i = 0; i < 6; i++) {
                                ...
                                float threshold = threshold_base[i] * abs(AXIS_SPEED[i]);
                                
							    	if (err >= threshold) {
                                        farPoint=true;
                                        float relative = (threshold/err);
                                        cap = FcMin+(FcMax-FcMin)*relative;
						...
						if (size == -1) {
							...
							for (int i = 0; i < 6; i++)
							{
								if (sinAxisEnabled[i])
								{
                                    ...
                                    AXIS_SPEED[i] = deltaPositions[i]*360.0/txRate[i];
